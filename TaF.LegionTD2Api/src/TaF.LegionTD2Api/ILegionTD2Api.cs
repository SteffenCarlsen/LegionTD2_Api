...
public interface ILegionTD2Api
{
    ...
    Task<List<PlayersData>> GetPlayerStatsByStatistics(int limit = 100, int offset = 0, PlayerSorting playerSorting = PlayerSorting.OverallElo, SortDirection sortDirection = SortDirection.Descending);
    ...
    Task<Ability> GetAbilityById(string id);
    Task<List<Ability>> GetAbilities(int limit = 10, int offset = 0);
    
    Task<Research> GetResearchById(string id);
    Task<Description> GetDescriptionById(string id);
    Task<List<Research>> GetResearch(int limit = 10, int offset = 0);
}